...
def test_no_grad_context_manager():
    """no_grad should function as both a context manager and decorator."""
...
def test_layernorm_layer():
    """Test LayerNorm normalizes mean=0, std=1 across feature dimensions."""
    ln = LayerNorm(normalized_shape=(4,))
    x = Tensor(np.random.randn(2, 3, 4), requires_grad=True)
    out = ln(x)

    assert out.data.shape == (2, 3, 4)
    # Means across last axis should be close to 0 (before affine gamma/beta)
